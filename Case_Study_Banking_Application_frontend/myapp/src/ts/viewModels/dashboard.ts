...
import 'oj-c/form-layout';
import "ojs/ojknockout";
import "mycomponent-1"
class TransferFundsViewModel {
    senderAccountId = ko.observable<string>("");
    receiverAccountId = ko.observable<string>("");
    amount = ko.observable<number>(0);
    transferStatus = ko.observable<string>("");

    submitTransfer = async () => {
        const senderAccountId = this.senderAccountId();
        const receiverAccountId = this.receiverAccountId();
        const amount = this.amount();

        // Validate input values
        if (!senderAccountId || !receiverAccountId || amount <= 0) {
            this.transferStatus("Please enter valid values for all fields.");
            return;
        }

        try {
            const response = await fetch("http://localhost:8080/customerapi/transfer", {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                this.transferStatus(`Error: ${error}`);
                return;
            }

            const data = await response.json();
            console.log('Transaction successful:', data);
            this.transferStatus(`Transaction Successful! Transaction ID: ${data.transactionId}`);
        } catch (error) {
            console.error('Error in transfer request:', error);
            this.transferStatus('An error occurred while processing the transaction. Please try again.');
        }
    };
}

whenDocumentReady().then(() => {
    const viewModel = new TransferFundsViewModel();
    ko.applyBindings(viewModel, document.getElementById('transfer-container'));
});

export = TransferFundsViewModel;
